...
namespace MPM.GUI
{
    public partial class FormPlotSamplesEM : ToolWindow
    {
        private const int PLOT_TIME_RANGE_EM = 10;  // seconds
        private const int DECIMATION_NORMAL = 4;  // 25%
        private const int MAX_CHART_POINTS =  CDetect.RAW_MP_SAMPLE_SIZE / DECIMATION_NORMAL * 2 * PLOT_TIME_RANGE_EM;
        ...
        private float m_fEMPlotMaxValue = float.MinValue;
        private float m_fEMPlotMinValue = float.MaxValue;
        //private int m_iAutoZoomInCounter;

        //private bool m_bIsLowRes = false;
       
        private float m_fEMSmoothData = 0.0f;
        ...
        public FormPlotSamplesEM(ref CDPointLookupTable DPointTable_)
        {
            InitializeComponent();
        ...
        //public void SetLowRes(bool bVal_)
        //{
        //    m_bIsLowRes = bVal_;
        //}
        ...
        public void Plot(CEventRawMPSamples evRaw_)
        {
            ...
            {                                              
                ...
                {                    
                    ...
                    {
                        fCurrentVal = m_iUnitSet != CCommonTypes.UNIT_SET.UNIT_SET_TO_METRIC ? evRaw_.m_shArrSample[i] * 0.145038f : evRaw_.m_shArrSample[i];

                        if (fCurrentVal > m_fEMPlotMaxValue)
                            m_fEMPlotMaxValue = fCurrentVal;
                        else if (fCurrentVal < m_fEMPlotMinValue)
                            m_fEMPlotMinValue = fCurrentVal;
                       
                        // inline low pass filter to speed up code
                        fLowPassVal = m_fEMSmoothData = m_fEMSmoothData - (m_fLPFBeta * (m_fEMSmoothData - fCurrentVal));
                    ...
                    if (m_fEMPlotMaxValue > chart1.ChartAreas[0].AxisY.Maximum)
                        chart1.ChartAreas[0].AxisY.Maximum = m_fEMPlotMaxValue;

                    if (m_fEMPlotMinValue < chart1.ChartAreas[0].AxisY.Minimum)
                        chart1.ChartAreas[0].AxisY.Minimum = m_fEMPlotMinValue;
                    ...
                    {
                        if (dpMax.YValues[0] < m_fEMPlotMaxValue)
                            chart1.ChartAreas[0].AxisY.Maximum = dpMax.YValues[0];

                        if (dpMin.YValues[0] > m_fEMPlotMinValue)
                ...
                catch (Exception ex)
                {
                    ...
                    System.Diagnostics.Debug.Write("Plot EM Data Error: " + ex.Message);
                }                                
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.Write("Plot EM Error: " + ex.Message);
                MessageBox.Show(ex.Message);
            }
        }
        ...
        public void GetSamples(object sender, CEventRawMPSamples evRaw_)
        {
            ...
            {                                                                                        
                ...
                {                        
                    ...
                    double removeBefore = dt.AddSeconds(-PLOT_TIME_RANGE_EM).ToOADate();  // subtract seconds
                    ...
                    chart1.ChartAreas[0].AxisX.Minimum = chart1.Series[0].Points[0].XValue;
                    chart1.ChartAreas[0].AxisX.Maximum = DateTime.FromOADate(chart1.Series[0].Points[0].XValue).AddSeconds(PLOT_TIME_RANGE_EM).ToOADate();
                ...
                {
                    //MessageBox.Show(ex.Message);
                    System.Diagnostics.Debug.Write("GetSamples Inner Try Error: " + ex.Message);
                }
            ...
            {
                System.Diagnostics.Debug.Write("GetSamples Outer Try Error: " + ex.Message);
                MessageBox.Show(ex.Message);
            }
        }
