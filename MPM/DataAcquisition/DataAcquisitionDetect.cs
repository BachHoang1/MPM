...
//using EMHDDSysConfig;
//using DetectSocketData;
//using ConstantDefines;
//using LocatorPositionData;

namespace MPM.DataAcquisition
{
    public class DataAcqDetect
    {
        ...
        //1/12/22
        //RunWorkerCompletedEventHandler m_eventWriteCompleted;
        //DoWorkEventHandler m_eventWriteDoWork;
        //BackgroundWorker _WriteBckGrdWorker = new BackgroundWorker();
        ////1/12/22
        ...
        //1/5/22FormPlotSamples m_frmPlotSamples;
        //1/5/22FormPlotSamplesEM m_frmPlotSamplesEM;
        //1/5/22FormFFT m_frmFFT;
        //1/5/22FormFFTEM m_frmFFTEM;
        //1/5/22CPumpPressure m_pumpPressure;

        public DataAcqDetect(CCommonTypes.TELEMETRY_TYPE tt_, ref DbConnection dbCnn_)
        {
            m_ttID = tt_;
            m_dbCnn = dbCnn_;
        ...
        public DataAcqDetect(CCommonTypes.TELEMETRY_TYPE tt_)
        {
            m_ttID = tt_;
            //m_dbCnn = dbCnn_;
            m_sIPAddress = DEFAULT_DETECT_IP_ADDRESS;
        ...
        */ //1/5/22
        ...
        void BackgroundInputProcessor_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            bool bSuccess = _dataReceiver.Start(); //2/21/22       
            ...
            while (!worker.CancellationPending)
            {
                ...
                ParsedDataPacket pdp = _dataReceiver.AcquireNextPacket();
                // ParsedDataPacket pdp = null;
                // bool result = _dataReceiver.NextPacket();
                //1/14/22
                if (pdp == null)
                //if (!result)
                {
                    m_iPDPNullCount++;
                    if (m_iPDPNullCount > 50)
                    {
                        Thread.Sleep(1000);
                        m_iPDPNullCount = 0;
                ...
                if (m_bExit)
                    evDetectConnected.m_iConnected = CEventDetect.CONNECTION.CLOSED;
                else
                    evDetectConnected.m_iConnected = _dataReceiver.IsConnected ? CEventDetect.CONNECTION.OPEN : CEventDetect.CONNECTION.CLOSED;
                ...
                //if (previousDetectConnectStatus != evDetectConnected.m_iConnected)
                //{
                //    LocatorDataSingleton lds = LocatorDataSingleton.Instance; //1/13/22
                //
                //    DetectSocketDataSingleton dsds = DetectSocketDataSingleton.Instance;
                //
                //    EMHDDSysConfigSingleton EMHDDSCS = EMHDDSysConfigSingleton.Instance;
                //
                //    SysWarningPerformedEventArgs sysWarningPerformedEventArgs = new SysWarningPerformedEventArgs();
                //
                //    if(evDetectConnected.m_iConnected == CEventDetect.CONNECTION.OPEN)
                //    {
                //        sysWarningPerformedEventArgs.Message = "Detect Socket Reconnected.";
                //        dsds.DetectSockData.ConnectionStatus = Status.CONNECTED;
                //    }
                //    else
                //    {
                //        sysWarningPerformedEventArgs.Message = "Detect Socket Disconnected.";
                //        dsds.DetectSockData.ConnectionStatus = Status.DISCONNECTED;
                //    }
                //           
                //    if (EMHDDSCS.EMHDDSysConfig.MainWindowConfig.SysWarningPerformedEvent != null)
                //    {
                //        EMHDDSCS.EMHDDSysConfig.MainWindowConfig.SysWarningPerformedEvent(this, sysWarningPerformedEventArgs);
                //    }
                //
                //    //Do no need to this the following
                //    //because when dsds.DetectSockData.ConnectionStatus is set
                //    //it will do the following automatially
                //    //if (dsds.DetectSockData.SocketStatusEvent != null)
                //    //{
                //        //dsds.DetectSockData.SocketStatusEvent(this, null);
                //    //}
                //}
                //
                //previousDetectConnectStatus = evDetectConnected.m_iConnected;
                //1/8/22
            }
        }

        //1/12/22
        //void WriteBckGrdWorker_DoWork(object sender, DoWorkEventArgs e)
        //{
        //    BackgroundWorker worker = sender as BackgroundWorker;
        //
        //    if (!_dataReceiver.IsConnected)
        //    {
        //        bool bSuccess = _dataReceiver.Start();
        //    }
        //
        //    _dataReceiver.WriteToNetwork();
        //
        //}
        ////1/12/22
        ...
        public void SetupBackgroundWorker()
        {
            ...
            //1/12/22          
            //m_eventWriteCompleted = new RunWorkerCompletedEventHandler(WriteBckGrdWorker_RunWorkerCompleted);
            //m_eventWriteDoWork = new DoWorkEventHandler(WriteBckGrdWorker_DoWork);
            //
            //_WriteBckGrdWorker.RunWorkerCompleted += m_eventWriteCompleted;
            //_WriteBckGrdWorker.DoWork += m_eventWriteDoWork;
            //1/12/22
        ...
        {
            Console.WriteLine("BackgroundInputProcessor_RunWorkerCompleted"); //2/21/22

            StartDataReceiverInBackground(m_sIPAddress, m_iPortNumber);
        }
        //1/12/22

        ///void CleanSocketData()
        //{
        //    DetectSocketDataSingleton dsds = DetectSocketDataSingleton.Instance;
        //    dsds.DetectSockData.SockeWriteBufferIndex = 0;
        //
        //    for (int i = 0; i < dsds.DetectSockData.SockeWriteBuffer.Length; i++)
        //    {
        //        dsds.DetectSockData.SockeWriteBuffer[i] = 0;
        //    }
        //}
        //void WriteBckGrdWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        //{
        //    CleanSocketData();
        //}
        //1/12/22
        ...
        public void EndDataReceiverInBackground()
        {
            ...
            _backgroundInputProcessor.CancelAsync();
            //_WriteBckGrdWorker.CancelAsync(); //1/12/22
        ...
        public void UnsetBackgroundWorker()
        {
            ...
            //_WriteBckGrdWorker.RunWorkerCompleted -= m_eventWriteCompleted; //1/12/22
            //_WriteBckGrdWorker.DoWork -= m_eventWriteDoWork; //1/12/22
        ...
        public void RefreshIPAddress(string sVal_, int iPortNumber_)
        {
            ...
            //{
            _dataReceiver.CloseConnection();
            _dataReceiver.RefreshIPAddress(m_sIPAddress, m_iPortNumber);
            _dataReceiver.Start();
            //}
            //else
            //{
        ...
        void BackgroundInputProcessor_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (IsConnected())
            {
                ...
                //1/10/22if (true/*JobPathRunManager.CurrentBitRun != null*/)
                //1/10/22RouteDataPackets(pdp);
            }
        }
                    ...
                    if (sDesc.Contains("TF Type"))   // sometimes there isn't an "=" sign to split
                    {
                        if (sDesc.Contains("TFO"))  // parse the sub value
                        {
                            ...
                            if (infoSub.iMessageCode > -1)
                            {
                                ...
                                //5/1/22 LogData(infoSub, DateTime.Now, sSubVal, false);
                            }
                        }
        ...
        */ //1/5/22
        ...
        public void RouteDataPackets(ParsedDataPacket pdp)
        {
            ...
            //DataPackets.Add(pdp);
            System.Diagnostics.Debug.WriteLine("CMD " + pdp.Command.ToString() + " " + pdp.UnicodeString);
            switch (pdp.Command)
            {
                ...
                case Command.COMMAND_RESP_A_PRESSURE:
                    ...
                    try
                    {
                        ...
                        if (m_ECD.IsReady())
                        {
                            ...
                            // save to db
                            //1/5/22 SaveECDToDB(fECD, fHSP, m_ECD.GetLengthUnit() == "m");
                        }
                    }
                    catch (Exception e)
                    {
                ...
                case Command.COMMAND_RESP_PACKET_TYPE:
                    ...
                    {
                        //1/5/22MessageBox.Show("Error: Possibly no ECD object defined. Restart Display: " + ex.Message, "ECD Validation", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                    //}
                    break;
        ...
        //    try
        //    {
        //        _WriteBckGrdWorker.RunWorkerAsync();
        //    }
        //    catch (Exception e2)
        //    {
        //        EMHDDSysConfigSingleton EMHDDSCS = EMHDDSysConfigSingleton.Instance;
        ...
        //        SysWarningPerformedEventArgs sysWarningPerformedEventArgs = new SysWarningPerformedEventArgs();
        //
        //        sysWarningPerformedEventArgs.Message = "WriteBckGrdWorker FAILED. Exception.Message=" + e2.Message;
        //
        //        if (EMHDDSCS.EMHDDSysConfig.MainWindowConfig.SysWarningPerformedEvent != null)
        //        {
        //            EMHDDSCS.EMHDDSysConfig.MainWindowConfig.SysWarningPerformedEvent(this, sysWarningPerformedEventArgs);
        ...
        //}
        //
        byte[] GetBigEndianBytes(UInt32 val, bool isLittleEndian)
        {
        ...
        /*
        byte[] GetBigEndianBytes(UInt32 val)
        {
